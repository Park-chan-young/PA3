...
def train():
    ...
    for epoch in range(num_epochs):
        for rgb, sparse, gt_depth, gt_normal in dataloader:
            rgb = rgb.cuda()
            sparse = sparse.cuda()
            ...
            # Loss_sparse
            mask = (sparse > 0).float()
            loss_sparse = (mask * torch.abs(pred_depth - gt_depth)).sum() / (mask.sum() + 1e-6)

            # Loss_normal
            pred_normal = depth_2_normal(pred_depth)
            loss_normal = normal_l2_loss(pred_normal, gt_normal.unsqueeze(0).cuda())
